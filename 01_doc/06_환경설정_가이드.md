# CDSS 프로젝트 환경 설정 가이드

**작성일**: 2025-12-19
**버전**: 1.0

---

## 📋 목차

1. [Python 가상환경 관리](#1-python-가상환경-관리)
2. [Node.js 패키지 관리](#2-nodejs-패키지-관리)
3. [환경 변수 설정](#3-환경-변수-설정)
4. [Docker 환경 설정](#4-docker-환경-설정)
5. [팀 협업 규칙](#5-팀-협업-규칙)

---

## 1. Python 가상환경 관리

### 1.1 가상환경 생성 및 활성화

```bash
# 프로젝트 디렉토리 이동
cd cdss-backend

# 가상환경 생성
python -m venv venv

# 가상환경 활성화
# Windows
venv\Scripts\activate

# macOS/Linux
source venv/bin/activate

# 활성화 확인 (프롬프트 앞에 (venv) 표시됨)
which python  # macOS/Linux
where python  # Windows
```

### 1.2 requirements.txt 관리 (필수!)

#### 초기 설치 후 requirements.txt 생성
```bash
# pip 최신 버전 업그레이드
pip install --upgrade pip

# 필요한 패키지 설치
pip install django==4.2 djangorestframework djangorestframework-simplejwt
pip install django-cors-headers celery redis pymysql
pip install channels channels-redis
pip install requests python-dotenv
pip install pytest pytest-django django-prometheus

# ⭐ 중요: requirements.txt 생성 (습관화!)
pip freeze > requirements.txt
```

#### 새로운 패키지 설치 시
```bash
# 1. 패키지 설치
pip install <package-name>

# 2. 즉시 requirements.txt 업데이트 (습관화!)
pip freeze > requirements.txt

# 3. Git에 커밋
git add requirements.txt
git commit -m "chore: add <package-name> to requirements"
```

#### 다른 개발자가 프로젝트 클론 시
```bash
# 1. 가상환경 생성 및 활성화
python -m venv venv
source venv/bin/activate  # Windows: venv\Scripts\activate

# 2. requirements.txt로 패키지 설치
pip install -r requirements.txt

# 3. 설치 확인
pip list
```

### 1.3 requirements.txt 예시

```txt
# cdss-backend/requirements.txt

# Django Core
Django==4.2.0
djangorestframework==3.14.0
djangorestframework-simplejwt==5.3.0

# Database
PyMySQL==1.1.0
mysqlclient==2.2.0

# CORS
django-cors-headers==4.3.0

# Async & Celery
celery==5.3.4
redis==5.0.0

# WebSocket
channels==4.0.0
channels-redis==4.1.0

# HTTP Requests
requests==2.31.0

# Environment Variables
python-dotenv==1.0.0

# Testing
pytest==7.4.3
pytest-django==4.5.2

# Monitoring
django-prometheus==2.3.1

# Development Tools
black==23.11.0
flake8==6.1.0
```

### 1.4 가상환경 비활성화

```bash
deactivate
```

### 1.5 .gitignore 설정

```gitignore
# cdss-backend/.gitignore

# 가상환경
venv/
env/
ENV/

# Python
__pycache__/
*.py[cod]
*$py.class
*.so
.Python

# Django
*.log
db.sqlite3
db.sqlite3-journal
media/
staticfiles/

# 환경 변수
.env
.env.local
.env.*.local

# IDE
.vscode/
.idea/
*.swp
*.swo

# OS
.DS_Store
Thumbs.db
```

---

## 2. Node.js 패키지 관리

### 2.1 package.json 관리

#### 초기 설정 후 package.json 확인
```bash
cd cdss-frontend-react

# 프로젝트 생성 시 자동 생성됨
npx create-react-app . --template typescript

# package.json 확인
cat package.json
```

#### 새로운 패키지 설치 시
```bash
# 1. 패키지 설치 (자동으로 package.json 업데이트)
npm install <package-name>

# 개발 의존성 설치
npm install -D <package-name>

# 2. package-lock.json도 함께 커밋
git add package.json package-lock.json
git commit -m "chore: add <package-name>"
```

#### 다른 개발자가 프로젝트 클론 시
```bash
# 1. 프로젝트 디렉토리 이동
cd cdss-frontend-react

# 2. package.json 기반 패키지 설치
npm install

# 3. 설치 확인
npm list --depth=0
```

### 2.2 package.json 예시

```json
{
  "name": "cdss-frontend-react",
  "version": "0.1.0",
  "private": true,
  "dependencies": {
    "axios": "^1.6.0",
    "react": "^18.2.0",
    "react-dom": "^18.2.0",
    "react-router-dom": "^6.20.0",
    "zustand": "^4.4.7",
    "@headlessui/react": "^1.7.17",
    "@heroicons/react": "^2.0.18",
    "react-hook-form": "^7.48.2",
    "zod": "^3.22.4",
    "@hookform/resolvers": "^3.3.2",
    "date-fns": "^2.30.0",
    "recharts": "^2.10.0",
    "clsx": "^2.0.0",
    "tailwind-merge": "^2.0.0"
  },
  "devDependencies": {
    "@types/react": "^18.2.0",
    "@types/react-dom": "^18.2.0",
    "typescript": "^5.0.0",
    "tailwindcss": "^3.3.0",
    "postcss": "^8.4.0",
    "autoprefixer": "^10.4.0",
    "@tailwindcss/forms": "^0.5.7",
    "vitest": "^1.0.0",
    "@testing-library/react": "^14.0.0",
    "@testing-library/jest-dom": "^6.0.0",
    "@playwright/test": "^1.40.0"
  },
  "scripts": {
    "start": "react-scripts start",
    "build": "react-scripts build",
    "test": "vitest",
    "test:e2e": "playwright test",
    "lint": "eslint src --ext .ts,.tsx",
    "format": "prettier --write \"src/**/*.{ts,tsx}\""
  }
}
```

### 2.3 .gitignore 설정

```gitignore
# cdss-frontend-react/.gitignore

# Dependencies
node_modules/
/.pnp
.pnp.js

# Testing
/coverage

# Production
/build

# Environment
.env
.env.local
.env.development.local
.env.test.local
.env.production.local

# Logs
npm-debug.log*
yarn-debug.log*
yarn-error.log*

# IDE
.vscode/
.idea/

# OS
.DS_Store
Thumbs.db
```

---

## 3. 환경 변수 설정

### 3.1 Backend (.env 파일)

```bash
# cdss-backend/.env

# Django Settings
DEBUG=True
SECRET_KEY=your-secret-key-here-change-in-production
ALLOWED_HOSTS=localhost,127.0.0.1

# Database
DB_ENGINE=django.db.backends.mysql
DB_NAME=cdss_db
DB_USER=cdss_user
DB_PASSWORD=your_password_here
DB_HOST=localhost
DB_PORT=3306

# Redis
REDIS_URL=redis://localhost:6379/0

# RabbitMQ
RABBITMQ_URL=amqp://guest:guest@localhost:5672/

# OpenEMR API
OPENEMR_BASE_URL=http://localhost:8080
OPENEMR_API_TOKEN=your_openemr_token_here

# Orthanc DICOM
ORTHANC_BASE_URL=http://localhost:8042
ORTHANC_USERNAME=orthanc
ORTHANC_PASSWORD=orthanc

# HAPI FHIR
HAPI_FHIR_BASE_URL=http://localhost:8080/fhir
HAPI_FHIR_AUTH_TOKEN=your_fhir_token_here

# MinIO S3
MINIO_ENDPOINT=localhost:9000
MINIO_ACCESS_KEY=minioadmin
MINIO_SECRET_KEY=minioadmin
MINIO_BUCKET_NAME=cdss-ai-artifacts

# JWT Settings
JWT_ACCESS_TOKEN_LIFETIME=30  # minutes
JWT_REFRESH_TOKEN_LIFETIME=7  # days

# Email (선택)
EMAIL_BACKEND=django.core.mail.backends.smtp.EmailBackend
EMAIL_HOST=smtp.gmail.com
EMAIL_PORT=587
EMAIL_USE_TLS=True
EMAIL_HOST_USER=your-email@gmail.com
EMAIL_HOST_PASSWORD=your-app-password
```

### 3.2 Frontend (.env 파일)

```bash
# cdss-frontend-react/.env

# API Base URL
REACT_APP_API_BASE_URL=http://localhost:8000/api

# WebSocket URL
REACT_APP_WS_BASE_URL=ws://localhost:8000/ws

# Environment
REACT_APP_ENV=development

# Feature Flags (선택)
REACT_APP_ENABLE_DICOM_VIEWER=true
REACT_APP_ENABLE_AI_FEATURES=true
```

### 3.3 .env.example 파일 생성 (Git에 포함)

```bash
# cdss-backend/.env.example

# Django Settings
DEBUG=True
SECRET_KEY=change-this-in-production
ALLOWED_HOSTS=localhost,127.0.0.1

# Database
DB_ENGINE=django.db.backends.mysql
DB_NAME=cdss_db
DB_USER=cdss_user
DB_PASSWORD=CHANGE_ME
DB_HOST=localhost
DB_PORT=3306

# ... (실제 값은 제거하고 템플릿만 제공)
```

### 3.4 환경 변수 로드 (Python)

```python
# cdss-backend/cdss_backend/settings.py

import os
from dotenv import load_dotenv

# .env 파일 로드
load_dotenv()

# 환경 변수 사용
SECRET_KEY = os.getenv('SECRET_KEY', 'fallback-secret-key')
DEBUG = os.getenv('DEBUG', 'False') == 'True'

DATABASES = {
    'default': {
        'ENGINE': os.getenv('DB_ENGINE'),
        'NAME': os.getenv('DB_NAME'),
        'USER': os.getenv('DB_USER'),
        'PASSWORD': os.getenv('DB_PASSWORD'),
        'HOST': os.getenv('DB_HOST'),
        'PORT': os.getenv('DB_PORT'),
    }
}
```

---

## 4. Docker 환경 설정

### 4.1 Dockerfile (Backend)

```dockerfile
# cdss-backend/Dockerfile

FROM python:3.10-slim

WORKDIR /app

# 시스템 의존성 설치
RUN apt-get update && apt-get install -y \
    gcc \
    default-libmysqlclient-dev \
    && rm -rf /var/lib/apt/lists/*

# Python 의존성 설치
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# 애플리케이션 코드 복사
COPY . .

# 포트 노출
EXPOSE 8000

# 실행 명령
CMD ["gunicorn", "cdss_backend.wsgi:application", "--bind", "0.0.0.0:8000"]
```

### 4.2 Dockerfile (Frontend)

```dockerfile
# cdss-frontend-react/Dockerfile

# Stage 1: Build
FROM node:18-alpine AS build

WORKDIR /app

# 의존성 설치
COPY package.json package-lock.json ./
RUN npm ci

# 빌드
COPY . .
RUN npm run build

# Stage 2: Production
FROM nginx:alpine

# 빌드된 파일 복사
COPY --from=build /app/build /usr/share/nginx/html

# Nginx 설정 복사 (선택)
# COPY nginx.conf /etc/nginx/conf.d/default.conf

EXPOSE 80

CMD ["nginx", "-g", "daemon off;"]
```

### 4.3 docker-compose.yml

```yaml
# docker-compose.yml

version: '3.8'

services:
  mysql:
    image: mysql:8.0
    environment:
      MYSQL_DATABASE: cdss_db
      MYSQL_USER: cdss_user
      MYSQL_PASSWORD: your_password
      MYSQL_ROOT_PASSWORD: root_password
    volumes:
      - mysql_data:/var/lib/mysql
    ports:
      - "3306:3306"

  redis:
    image: redis:7-alpine
    ports:
      - "6379:6379"

  rabbitmq:
    image: rabbitmq:3-management
    ports:
      - "5672:5672"
      - "15672:15672"

  django:
    build: ./cdss-backend
    command: gunicorn cdss_backend.wsgi:application --bind 0.0.0.0:8000
    volumes:
      - ./cdss-backend:/app
    ports:
      - "8000:8000"
    env_file:
      - ./cdss-backend/.env
    depends_on:
      - mysql
      - redis
      - rabbitmq

  react:
    build: ./cdss-frontend-react
    ports:
      - "3000:80"
    depends_on:
      - django

volumes:
  mysql_data:
```

---

## 5. 팀 협업 규칙

### 5.1 의존성 관리 체크리스트

#### Backend (Python)
- [ ] 새 패키지 설치 후 즉시 `pip freeze > requirements.txt` 실행
- [ ] requirements.txt를 Git에 커밋
- [ ] 다른 팀원에게 슬랙/이메일로 의존성 업데이트 알림
- [ ] 주요 버전 업그레이드 시 CHANGELOG.md 업데이트

#### Frontend (Node.js)
- [ ] `npm install` 사용 (package.json 자동 업데이트)
- [ ] package.json과 package-lock.json 모두 Git에 커밋
- [ ] yarn 사용 시 yarn.lock도 커밋
- [ ] 주요 라이브러리 업그레이드 시 팀원에게 알림

### 5.2 환경 변수 관리

- [ ] `.env` 파일은 절대 Git에 커밋하지 않음
- [ ] `.env.example` 파일은 템플릿으로 Git에 포함
- [ ] 새로운 환경 변수 추가 시 `.env.example`도 업데이트
- [ ] 민감 정보 (API 키, 비밀번호)는 팀 공유 문서에 별도 관리

### 5.3 가상환경 관리

- [ ] `venv/` 디렉토리는 Git에 커밋하지 않음 (.gitignore 설정)
- [ ] 각 개발자는 로컬에서 독립적인 가상환경 생성
- [ ] 가상환경 활성화 상태에서 개발 진행
- [ ] 프로젝트 클론 시 README.md의 설치 가이드 참조

### 5.4 Docker 환경

- [ ] Dockerfile 변경 시 팀원에게 알림
- [ ] docker-compose.yml 업데이트 시 `docker-compose up --build` 재실행 필요
- [ ] 환경 변수는 `.env` 파일로 관리 (docker-compose.yml에 하드코딩 금지)

### 5.5 커밋 메시지 규칙

```bash
# 의존성 추가
git commit -m "chore: add axios for API requests"

# 의존성 업그레이드
git commit -m "chore: upgrade django from 4.1 to 4.2"

# 의존성 제거
git commit -m "chore: remove unused package moment.js"

# requirements.txt 업데이트
git commit -m "chore: update requirements.txt"
```

---

## 📝 빠른 참조

### Python 의존성 관리
```bash
# 설치 후 즉시
pip install <package>
pip freeze > requirements.txt
git add requirements.txt
git commit -m "chore: add <package>"

# 다른 개발자
pip install -r requirements.txt
```

### Node.js 의존성 관리
```bash
# 설치 후 자동 업데이트
npm install <package>
git add package.json package-lock.json
git commit -m "chore: add <package>"

# 다른 개발자
npm install
```

### 환경 변수
```bash
# .env.example 복사
cp .env.example .env

# 실제 값 입력 (에디터로)
nano .env
```

### Docker
```bash
# 빌드 및 실행
docker-compose up --build

# 백그라운드 실행
docker-compose up -d

# 중지
docker-compose down
```

---

## ⚠️ 주의사항

### 절대 하지 말아야 할 것
1. ❌ `.env` 파일을 Git에 커밋
2. ❌ `venv/` 디렉토리를 Git에 커밋
3. ❌ `node_modules/` 디렉토리를 Git에 커밋
4. ❌ API 키, 비밀번호를 코드에 하드코딩
5. ❌ requirements.txt 업데이트 없이 새 패키지 사용

### 반드시 해야 할 것
1. ✅ 패키지 설치 후 즉시 requirements.txt 또는 package.json 업데이트
2. ✅ .env.example 파일 최신 상태 유지
3. ✅ .gitignore 파일 설정 확인
4. ✅ 가상환경 활성화 상태에서 개발
5. ✅ 의존성 변경 시 팀원에게 알림

---

## 6. 개발 모드 설정

### 6.1 보안/권한 토글 기능

개발 편의성을 위해 **보안/권한 기능을 선택적으로 활성화/비활성화**할 수 있습니다.

#### 개발 모드 (기본값)

**.env 파일 설정:**
```env
ENABLE_SECURITY=False  ← 기본값 (개발 모드)
```

**효과:**
- ✅ 인증 없이 모든 API 접근 가능
- ✅ 권한 검증 우회 (IsAdmin, IsDoctor 등 무시)
- ✅ 빠른 테스트 및 디버깅
- ⚠️ 감사 로그는 LOGIN/LOGOUT만 기록

**사용 예시:**
```bash
# 토큰 없이 바로 API 호출!
curl http://localhost:8000/api/acct/me/
curl http://localhost:8000/api/emr/patients/
```

#### 프로덕션 모드

**.env 파일 설정:**
```env
ENABLE_SECURITY=True
```

**효과:**
- 🔒 Token 인증 필수
- 🔒 역할 기반 권한 검증 (RBAC)
- 🔒 전체 감사 로그 기록
- ⚠️ 프로덕션 배포 시 반드시 활성화!

### 6.2 모드 전환 방법

```bash
# 1. .env 파일 수정
# ENABLE_SECURITY=False → True (또는 반대)

# 2. Django 재시작
python manage.py runserver

# 3. 로그 확인
# 🔒 SECURITY MODE: ENABLED (프로덕션)
# ⚠️ SECURITY MODE: DISABLED (개발)
```

### 6.3 settings.py 핵심 로직

```python
# 보안 설정 토글
ENABLE_SECURITY = os.getenv('ENABLE_SECURITY', 'False').lower() == 'true'

# REST Framework 권한 설정
if ENABLE_SECURITY:
    # 프로덕션: 인증 필수
    DEFAULT_PERMISSION_CLASSES = ["rest_framework.permissions.IsAuthenticated"]
else:
    # 개발: 인증 우회
    DEFAULT_PERMISSION_CLASSES = ["rest_framework.permissions.AllowAny"]

REST_FRAMEWORK = {
    "DEFAULT_PERMISSION_CLASSES": DEFAULT_PERMISSION_CLASSES,
    # ...
}
```

### 6.4 모드별 비교표

| 항목 | 개발 모드 | 프로덕션 모드 |
|------|----------|---------------|
| **인증** | 불필요 ✅ | 필수 🔒 |
| **권한** | 우회 ✅ | 검증 🔒 |
| **감사 로그** | 부분 (LOGIN/LOGOUT) | 전체 🔒 |
| **API 접근** | 자유 ✅ | 제한 🔒 |
| **토큰 만료** | 24시간 | 1시간 |
| **용도** | 개발/테스트 | 프로덕션 |

### 6.5 프로덕션 배포 체크리스트

```bash
# ❌ 절대 금지!
# 프로덕션에서 ENABLE_SECURITY=False는 심각한 보안 위험!

# ✅ 프로덕션 .env 설정 필수
ENABLE_SECURITY=True
DEBUG=False
SECRET_KEY=<강력한_시크릿_키>
```

### 6.6 Docker에서 개발 모드 설정

```yaml
# docker-compose.yml
services:
  django:
    environment:
      - ENABLE_SECURITY=false  # 개발 모드
      # ENABLE_SECURITY=true   # 프로덕션 모드
```

---

**Last Updated**: 2025-12-22
**Version**: 1.1
**Author**: Claude AI
