...
* **Nginx Gateway:** 외부 트래픽의 유일한 창구. SSL 종료 및 **X-Accel-Redirect**를 통한 내부 서비스(Orthanc) 보안 고도화.
...
* **Orthanc PACS Server:** HTJ2K 무손실 압축 DICOM 저장소. **외부망과 격리**되어 오직 Django/Nginx를 통해서만 접근 가능.
...
3.  **Nginx** $\rightarrow$ **Django CDSS Main Server:** (HTTP/REST) 인증 및 비즈니스 로직 처리.
4.  **이미지 요청:** Client $\rightarrow$ Nginx $\rightarrow$ Django(Signed URL 검증) $\rightarrow$ Nginx(Internal Redirect) $\rightarrow$ Orthanc $\rightarrow$ Client (HTJ2K 스트리밍).
5.  **UI Layer** $\leftrightarrow$ **Django Channels (Daphne):** WebSocket Connection (Real-time Notifications).
...
### 3.4 의료 데이터 연동 및 보안 (Medical Connectivity & Security)
*   **Django** $\leftrightarrow$ **HAPI FHIR:** 환자 동기화 (Patient Sync), AI 결과 표준화 저장.
*   **이미지 보안 서빙:** Django가 이미지 접근 권한 확인 후 **Signed URL** 생성 $\rightarrow$ Nginx가 내부적으로 Orthanc 데이터를 인출하여 전달.
*   **Orthanc Server:** 외부망 직접 노출 차단. 모든 입/출력은 HTJ2K 포맷으로 최적화.
