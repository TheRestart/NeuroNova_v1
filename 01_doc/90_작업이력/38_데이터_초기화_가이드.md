# ~~38. 데이터 초기화 가이드~~ [DEPRECATED]

**⚠️ 본 문서는 폐기되었습니다.**

**폐기일**: 2026-01-01
**대체 문서**: [초기_데이터_시딩_가이드.md](초기_데이터_시딩_가이드.md)

**폐기 사유**:
- OpenEMR 매핑 필드 (`openemr_patient_id`) 누락
- OpenEMR 데이터베이스 초기화 방법 미포함
- 데이터 아키텍처 (43, 44번 문서)를 준수하지 않음
- Outbox 패턴 미반영

**신규 가이드 참조**: [초기_데이터_시딩_가이드.md](초기_데이터_시딩_가이드.md)

---

**원본 문서 (참고용)**

---

**작성일**: 2025-12-30
**버전**: 2.0 (docker-compose.dev.yml 통합 환경 기준) [Deprecated]
**목적**: Docker 통합 환경에서 샘플 데이터를 빠르게 초기화하는 방법

---

## 📋 목차

1. [개요](#개요)
2. [전제 조건](#전제-조건)
3. [빠른 시작 (3단계)](#빠른-시작-3단계)
4. [데이터 초기화 스크립트](#데이터-초기화-스크립트)
5. [트러블슈팅](#트러블슈팅)

---

## 🎯 개요

NeuroNova CDSS 통합 Docker 환경에서 개발 및 테스트를 위한 샘플 데이터를 초기화합니다.

**지원 환경**:
- ✅ **docker-compose.dev.yml** (14개 컨테이너 통합 환경) - 권장
- ⚠️ 레거시 개별 Docker + 로컬 Django (더 이상 권장하지 않음)

---

## 🔧 전제 조건

### Docker 통합 환경 실행

전체 스택이 실행 중이어야 합니다:

```powershell
# 루트 디렉토리에서 실행
cd d:\1222\NeuroNova_v1

# 전체 스택 시작 (14개 컨테이너)
docker-compose -f docker-compose.dev.yml up -d

# 컨테이너 상태 확인 (모두 healthy 또는 running 상태 확인)
docker-compose -f docker-compose.dev.yml ps
```

**필수 서비스 확인**:
- ✅ django (healthy)
- ✅ cdss-mysql (healthy)
- ✅ orthanc (healthy)
- ✅ redis (healthy)
- ✅ celery-worker (running)

---

## 🚀 빠른 시작 (3단계)

### Step 1: 테스트 사용자 생성

```powershell
# Django 컨테이너에서 실행
docker-compose -f docker-compose.dev.yml exec django python manage.py create_test_users
```

**출력 예시**:
```
[SUCCESS] Updated user: admin (admin)
[SUCCESS] Updated user: doctor (doctor)
[SUCCESS] Updated user: nurse (nurse)
...
[SUMMARY] 13 users updated
```

**생성되는 계정**:
| 역할 | Username | Password | 비고 |
|------|----------|----------|------|
| Admin | admin | admin123!@# | 시스템 관리자 |
| Doctor | doctor | doctor123!@# | 의사 |
| Nurse | nurse | nurse123!@# | 간호사 |
| Patient | patient | patient123!@# | 환자 |
| Radiologist | radiologist | rib123!@# | 영상의학과 |
| Lab Tech | labtech | lab123!@# | 임상병리사 |

---

### Step 2: 샘플 데이터 초기화

```powershell
# 기존 데이터 유지하고 추가
docker-compose -f docker-compose.dev.yml exec django python manage.py init_sample_data

# 또는 기존 데이터 삭제 후 재생성
docker-compose -f docker-compose.dev.yml exec django python manage.py init_sample_data --reset
```

**생성되는 데이터**:
- 👥 **사용자 7명**: admin, doctor1, doctor2, radiologist1, nurse1, tech1, researcher1
- 🏥 **환자 5명**: P20250001 ~ P20250005 (Kim MinJun, Lee SeoYun, Park JiHoon, Choi YeJin, Jung DoHyun)

**출력 예시**:
```
[START] Sample data initialization...
[USERS] Creating users...
   [OK] User created: admin (admin)
   [OK] User created: doctor1 (doctor)
   ...
[PATIENTS] Creating patients...
   [OK] Patient created: P20250001 - Kim MinJun
   ...

============================================================
[SUMMARY] Sample Data
============================================================

[USERS] Created/Updated: 7
   - admin: admin
   - doctor: doctor1
   ...

[PATIENTS] Created: 5
   - P20250001: Kim MinJun
   - P20250002: Lee SeoYun
   ...

[DB] Total Patients: 5
[DB] Total Users: 13

============================================================
[COMPLETE] All done!
============================================================
```

---

### Step 3: DICOM 샘플 업로드 (선택사항)

```powershell
# Dry-run 모드로 파일 검사만 (실제 업로드 안 함)
docker-compose -f docker-compose.dev.yml exec django python manage.py upload_sample_dicoms --dry-run

# 실제 업로드
docker-compose -f docker-compose.dev.yml exec django python manage.py upload_sample_dicoms
```

**검출된 파일**: 1,860개 DICOM 파일 (sub-0004, sub-0005)

**출력 예시**:
```
[INFO] Scanning DICOM files in: /app/../sample_dicoms
[INFO] Orthanc URL: http://orthanc:8042
[DRY RUN] Files will not be uploaded

[INFO] Scanning sub-0004...
[INFO] Scanning sub-0005...

[SUMMARY] Found 1,860 DICOM files
[STATS] sub-0004: 930 files
[STATS] sub-0005: 930 files

[DRY RUN] Skipped uploading 1,860 files
```

---

## 📦 데이터 초기화 스크립트

### 1. create_test_users.py

**파일 위치**: `acct/management/commands/create_test_users.py`

**기능**:
- 7개 역할별 테스트 사용자 생성 (admin, doctor, nurse, patient, radiologist, labtech, external)
- 총 13개 사용자 (React 테스트용 + 원본 테스트용)

**사용법**:
```powershell
# Docker 환경
docker-compose -f docker-compose.dev.yml exec django python manage.py create_test_users

# 레거시 (로컬 Django)
python manage.py create_test_users
```

---

### 2. init_sample_data.py

**파일 위치**: `acct/management/commands/init_sample_data.py`

**기능**:
- 7개 역할 사용자 생성 (doctor1, doctor2, radiologist1, nurse1, tech1, researcher1, admin)
- 5개 환자 생성 (P20250001 ~ P20250005)
- 환자 상세 정보 (연락처, 주소, 혈액형, 알러지)

**사용법**:
```powershell
# 기본 사용 (기존 데이터 유지)
docker-compose -f docker-compose.dev.yml exec django python manage.py init_sample_data

# 전체 재생성 (기존 데이터 삭제)
docker-compose -f docker-compose.dev.yml exec django python manage.py init_sample_data --reset
```

**Windows 호환성**:
- ✅ 모든 이모지 제거 (cp949 인코딩 오류 해결)
- ✅ ASCII 텍스트 마커 사용 ([INFO], [SUCCESS], [ERROR])

**생성되는 환자 데이터**:
| Patient ID | Name | Birth Date | Gender | Blood Type |
|------------|------|------------|--------|------------|
| P20250001 | Kim MinJun | 1985-05-15 | male | A+ |
| P20250002 | Lee SeoYun | 1990-08-22 | female | B+ |
| P20250003 | Park JiHoon | 1978-03-10 | male | O+ |
| P20250004 | Choi YeJin | 1995-11-30 | female | AB+ |
| P20250005 | Jung DoHyun | 1982-07-18 | male | A- |

---

### 3. upload_sample_dicoms.py

**파일 위치**: `ris/management/commands/upload_sample_dicoms.py`

**기능**:
- `sample_dicoms` 디렉토리의 DICOM 파일을 Orthanc PACS에 업로드
- 검출된 파일 수: 1,860개 (sub-0004, sub-0005)

**사용법**:
```powershell
# Dry-run 모드 (실제 업로드 안 함)
docker-compose -f docker-compose.dev.yml exec django python manage.py upload_sample_dicoms --dry-run

# 실제 업로드
docker-compose -f docker-compose.dev.yml exec django python manage.py upload_sample_dicoms
```

**Windows 호환성**:
- ✅ 이모지 제거 (cp949 호환)

---

## 🔧 트러블슈팅

### 1. Django 컨테이너에 접속할 수 없음

**증상**: `docker-compose exec django` 실패

**해결**:
```powershell
# 컨테이너 상태 확인
docker-compose -f docker-compose.dev.yml ps

# Django 컨테이너 로그 확인
docker-compose -f docker-compose.dev.yml logs django

# 컨테이너가 없다면 재시작
docker-compose -f docker-compose.dev.yml up -d django
```

---

### 2. ImportError: cannot import name 'UserProfile'

**증상**: init_sample_data.py 실행 시 ImportError 발생

**원인**: 구버전 스크립트 사용 중

**해결**: 최신 스크립트로 업데이트 (이미 적용됨)
- ✅ UserProfile, Role 모델 제거
- ✅ User, PatientCache 모델만 사용

---

### 3. UnicodeEncodeError (Windows)

**증상**: `'cp949' codec can't encode character`

**원인**: 스크립트에 이모지 문자 포함

**해결**: 최신 스크립트로 업데이트 (이미 적용됨)
- ✅ 모든 이모지를 ASCII 텍스트로 변경

---

### 4. Orthanc 연결 실패

**증상**: Connection refused to Orthanc

**해결**:
```powershell
# Orthanc 컨테이너 상태 확인
docker-compose -f docker-compose.dev.yml ps orthanc

# Orthanc 헬스체크
docker-compose -f docker-compose.dev.yml exec orthanc curl http://localhost:8042/system

# Orthanc 로그 확인
docker-compose -f docker-compose.dev.yml logs orthanc
```

---

### 5. MySQL 연결 실패

**증상**: OperationalError: Can't connect to MySQL

**해결**:
```powershell
# MySQL 컨테이너 상태 확인
docker-compose -f docker-compose.dev.yml ps cdss-mysql

# MySQL 헬스체크
docker-compose -f docker-compose.dev.yml exec cdss-mysql mysqladmin ping -h localhost -u root -p

# MySQL 로그 확인
docker-compose -f docker-compose.dev.yml logs cdss-mysql
```

---

## 📝 완전 초기화 시나리오

Docker를 완전히 삭제하고 재설치한 경우:

```powershell
# 1. 루트 디렉토리로 이동
cd d:\1222\NeuroNova_v1

# 2. 전체 스택 시작 (14개 컨테이너)
docker-compose -f docker-compose.dev.yml up -d

# 3. 컨테이너 상태 확인 (모두 healthy 대기)
docker-compose -f docker-compose.dev.yml ps

# 4. Django 마이그레이션 (자동 실행됨, 확인용)
docker-compose -f docker-compose.dev.yml exec django python manage.py migrate

# 5. 테스트 사용자 생성
docker-compose -f docker-compose.dev.yml exec django python manage.py create_test_users

# 6. 샘플 데이터 생성
docker-compose -f docker-compose.dev.yml exec django python manage.py init_sample_data

# 7. DICOM 업로드 (선택사항)
docker-compose -f docker-compose.dev.yml exec django python manage.py upload_sample_dicoms --dry-run

# 완료!
```

**예상 시간**: 3-5분 (컨테이너 시작 시간 포함)

---

## 📚 관련 문서

- [39_Docker_아키텍처.md](39_Docker_아키텍처.md) - Docker 아키텍처 상세
- [40_Docker_개발_가이드.md](40_Docker_개발_가이드.md) - Docker 개발 실습
- [REF_CLAUDE_ONBOARDING_QUICK.md](REF_CLAUDE_ONBOARDING_QUICK.md) - 빠른 시작 가이드

---

**문서 버전**: 2.0 (docker-compose.dev.yml 통합)
**최종 수정**: 2025-12-30
**작성자**: NeuroNova Development Team
