# NeuroNova CDSS - 초기 데이터 시딩 가이드

**최종 수정일**: 2026-01-02
**버전**: 2.0
**목적**: Docker 통합 환경에서 데이터 아키텍처를 준수하여 초기 데이터를 정확하게 시딩하는 방법

**주요 변경 (v2.0)**:
- ✅ OpenEMR Skip 모드 추가 (OAuth2 이슈 우회)
- ✅ 자동 실행 스크립트 반영
- ✅ 최신 환경 변수 (.env) 업데이트

---

## 📋 목차

1. [개요](#개요)
2. [데이터 저장소 구조](#데이터-저장소-구조)
3. [빠른 시작 (4단계)](#빠른-시작-4단계)
4. [데이터 시딩 상세](#데이터-시딩-상세)
5. [검증 및 확인](#검증-및-확인)
6. [트러블슈팅](#트러블슈팅)

---

## 🎯 개요

NeuroNova CDSS는 **4개의 데이터 저장소**를 사용하며, 각 저장소는 명확한 역할과 책임을 가집니다.

```
┌────────────────────────────────────────────────────────────────────┐
│                    NeuroNova CDSS 데이터 저장소                      │
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────┐  ┌──────────┐    │
│  │ Django MySQL │  │ OpenEMR      │  │ HAPI     │  │ Orthanc  │    │
│  │ (Cache/Data) │  │ (EMR Detail) │  │ FHIR     │  │ (DICOM)  │    │
│  └──────────────┘  └──────────────┘  └──────────┘  └──────────┘    │
│                                                                    │
│  • 빠른 조회      • 상세 EMR      • 표준 교환    • 의료 영상            │
│  • 리스트        • 진료 기록      • 외부 연동    • PACS                │
│  • 유전 정보      • 처방 상세      • FHIR R4     • DICOM              │
│  • AI 결과                                                          │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

### 시딩 목표

- ✅ **Django MySQL**: 기본 정보 + 완전 데이터 (EMR 캐시, LIS 전체, AI 결과)
- ✅ **OpenEMR**: 환자 상세 정보 (과거 병력, 가족력 등)
- ✅ **HAPI FHIR**: 표준 교환 리소스 (자동 동기화)
- ✅ **Orthanc PACS**: DICOM 샘플 이미지

---

## 🏗️ 데이터 저장소 구조

### 1. Django MySQL (Cache + Full Data)

**저장 데이터**:
| 테이블 | 용도 | 데이터 특성 |
|--------|------|------------|
| `emr_patient_cache` | 환자 기본 정보 | **캐시** (OpenEMR과 동기화) |
| `emr_encounters` | 진료 기록 | **캐시** (OpenEMR과 동기화) |
| `ocs_orders` | 처방 주문 | **캐시** (OpenEMR과 동기화) |
| `lis_lab_results` | 검사 결과 | **완전 데이터** (유전 정보 포함) |
| `ai_aijob` | AI 추론 결과 | **완전 데이터** (텍스트/숫자) |
| `ris_radiology_study` | 영상 메타데이터 | **완전 데이터** (Orthanc 동기화) |

### 2. OpenEMR MySQL (EMR Detail)

**저장 데이터**:
- 환자 상세 정보 (과거 병력, 가족력, 사회력, 직업, 혈액형, 결혼 상태)
- 진료 기록 상세 (SOAP Note, 치료 계획)
- 처방 상세 정보 (약물 상호작용, 알레르기 체크)

**매핑 필드**:
- `PatientCache.openemr_patient_id` ↔ OpenEMR Patient ID

### 3. HAPI FHIR R4 (Standard Exchange)

**저장 데이터**:
- `Patient`: PatientCache에서 자동 동기화
- `Encounter`, `MedicationRequest` (TODO)

**동기화 방식**:
- Django signals (post_save, post_delete)로 자동

### 4. Orthanc PACS (DICOM Storage)

**저장 데이터**:
- DICOM 원본 이미지 (CT, MRI, X-Ray)
- AI 생성 Segmentation (의사 승인 후)

---

## 🚀 빠른 시작 (4단계)

### 전제 조건

```powershell
# 루트 디렉토리로 이동
cd d:\1222\NeuroNova_v1

# Docker 통합 환경 시작
docker-compose -f docker-compose.dev.yml up -d

# 컨테이너 상태 확인 (모두 healthy/running)
docker-compose -f docker-compose.dev.yml ps
```

---

### Step 1: 테스트 사용자 생성

```powershell
docker-compose -f docker-compose.dev.yml exec django python manage.py create_test_users
```

**생성되는 계정** (비밀번호 단순화 - v2.0):
| 역할 | Username | Password | 비고 |
|------|----------|----------|------|
| Admin | admin | admin123 | 시스템 관리자 |
| Doctor | doctor | doctor123 | 의사 |
| Nurse | nurse | nurse123 | 간호사 |
| Patient | patient | patient123 | 환자 |
| Radiologist | radiologist | radiologist123 | 영상의학과 |
| Lab Tech | labtech | labtech123 | 임상병리사 |

**변경 사항 (2026-01-02)**:
- 특수문자 제거 (`*123!@#` → `*123`)
- Python escape sequence 문제 해결
- 개발 편의성 향상

**출력 예시**:
```
[SUCCESS] Updated user: admin (admin)
[SUCCESS] Updated user: doctor (doctor)
...
[SUMMARY] 13 users updated
```

---

### Step 2: 최신 통합 테스트 데이터 시딩 (v3.0)

2026-01-02 업데이트된 시딩 스크립트를 사용하여 모든 모듈(UC01~UC05)을 한 번에 검증할 수 있습니다.

#### 옵션 A: 최소 데이터 시딩 (빠른 검증용)
핵심 환자와 유전 검사 결과, RIS 연동을 빠르게 확인하고 싶을 때 사용합니다.
```powershell
# Django 서버 디렉토리로 이동 후 실행
cd NeuroNova_02_back_end\02_django_server
python seed_minimal.py
```

#### 옵션 B: 전체 시스템 테스트 데이터 시딩
모든 UC(User Case)를 전수 조사하기 위한 상세 데이터를 생성합니다.
```powershell
python seed_full_system_test_data.py
```

**v3.0 시딩으로 보장되는 사항**:
- 🧬 **UC04 LIS**: `sub-0005` 환자의 **BRCA1 유전자 변이 데이터** (`LR-GENE-001`) 자동 생성
- 🖼️ **UC05 RIS**: Orthanc 서버와의 연동을 통한 최신 영상 메타데이터 동기화 및 뷰어 연동
- 🛠️ **DB 정합성**: Django 모델과 실제 MySQL 컬럼(예: `ordered_by`, `patient_id`) 간의 매핑 오류가 해결된 상태로 데이터가 저장됨

**출력 예시**:
```
============================================================
[START] Initial Data Seeding
============================================================

[STEP 1/4] Creating Users...
   [OK] User created: admin (admin)
   [OK] User created: doctor1 (doctor)
   ...

[STEP 2/4] Creating Patients (Django MySQL)...
   [OK] Patient created: P-2025-001 - Kim MinJun
   [INFO] openemr_patient_id: OPENEMR_P-2025-001
   [INFO] FHIR sync queued: uuid-abc-123
   ...

[STEP 3/4] Syncing to OpenEMR (Outbox pattern)...
   [INFO] 5 patients queued for OpenEMR sync
   [INFO] Celery worker will process in background

[STEP 4/4] Verifying FHIR sync...
   [OK] FHIR Patient created: fhir-123

============================================================
[SUMMARY] Data Seeding Complete
============================================================

[Django MySQL] PatientCache: 5 patients
[OpenEMR Sync] Outbox: 5 pending
[HAPI FHIR] Patient: 5 resources
[Users] Total: 13 users

============================================================
[NEXT STEPS]
============================================================
1. Wait 1-2 minutes for OpenEMR background sync
2. Verify sync status: GET /api/patients/
3. Upload DICOM samples (optional)

============================================================
```

---

### Step 3: OpenEMR 동기화 확인

```powershell
# Outbox 상태 확인
docker-compose -f docker-compose.dev.yml exec django python manage.py check_sync_status

# 수동 재시도 (필요 시)
docker-compose -f docker-compose.dev.yml exec django python manage.py retry_failed_outbox
```

**예상 출력**:
```
============================================================
[Sync Status Report]
============================================================

[Outbox Status]
   Pending: 0
   Processing: 0
   Done: 5
   Failed: 0

[Recent Syncs]
   P-2025-001 → OpenEMR: SUCCESS (openemr-patient-456)
   P-2025-002 → OpenEMR: SUCCESS (openemr-patient-457)
   P-2025-003 → OpenEMR: SUCCESS (openemr-patient-458)
   P-2025-004 → OpenEMR: SUCCESS (openemr-patient-459)
   P-2025-005 → OpenEMR: SUCCESS (openemr-patient-460)

============================================================
[RESULT] All syncs completed successfully
============================================================
```

---

### Step 4: DICOM 샘플 업로드 (선택사항)

```powershell
# Dry-run 모드 (파일 검사만)
docker-compose -f docker-compose.dev.yml exec django python manage.py upload_sample_dicoms --dry-run

# 실제 업로드
docker-compose -f docker-compose.dev.yml exec django python manage.py upload_sample_dicoms
```

**검출된 파일**: 1,860개 DICOM 파일 (sub-0004, sub-0005)

**출력 예시**:
```
[INFO] Scanning DICOM files in: /app/../sample_dicoms
[INFO] Orthanc URL: http://orthanc:8042

[INFO] Scanning sub-0004...
[INFO] Scanning sub-0005...

[SUMMARY] Found 1,860 DICOM files
[STATS] sub-0004: 930 files
[STATS] sub-0005: 930 files

[UPLOAD] Uploading to Orthanc...
[PROGRESS] 100/1860 (5%)
[PROGRESS] 500/1860 (27%)
[PROGRESS] 1000/1860 (54%)
[PROGRESS] 1500/1860 (81%)
[PROGRESS] 1860/1860 (100%)

[DONE] Successfully uploaded 1,860 DICOM files
```

---

## 📦 데이터 시딩 상세

### 시딩 스크립트: `seed_initial_data.py`

**파일 위치**: `acct/management/commands/seed_initial_data.py`

**기능**:
1. **사용자 생성**: 7개 역할 사용자
2. **환자 생성 (Django MySQL)**:
   - `PatientCache` 모델에 기본 정보 저장
   - `openemr_patient_id` 필드 설정 (예: `OPENEMR_P-2025-001`)
3. **Outbox 레코드 생성**:
   - OpenEMR 동기화 작업 예약
   - `emr_sync_outbox` 테이블에 저장
4. **FHIR 자동 동기화**:
   - Django signals가 자동으로 HAPI FHIR에 Patient 리소스 생성

### 환자 ID 형식 표준

**Django MySQL**:
- `patient_id`: `P-2025-001` (시스템 표준) 또는 `sub-0005` (DICOM 매핑용)

**HAPI FHIR 매핑**:
- `fhir_id`: `fhir-uuid-xxx` (자동 생성)

### 샘플 환자 데이터

| Patient ID | Name | Birth Date | Gender | Blood Type | OpenEMR ID | 비고 |
|------------|------|------------|--------|------------|------------|------|
| P-2025-001 | Kim MinJun | 1980-01-01 | male | O+ | OPENEMR_P-2025-001 | 핵심 샘플 |
| sub-0005 | Patient Five | 1985-05-05 | female | B- | OPENEMR_sub-0005 | PACS 연동용 |
| P-2025-002 | Lee SeoYun | 1990-08-22 | female | B+ | OPENEMR_P-2025-002 | |

---

## ✅ 검증 및 확인

### 1. Django MySQL 확인

```powershell
# Django shell 진입
docker-compose -f docker-compose.dev.yml exec django python manage.py shell

# Python shell에서 실행
from emr.models import PatientCache
from emr.models import SyncOutbox

# 환자 수 확인
print(f"Total patients: {PatientCache.objects.count()}")

# OpenEMR 매핑 확인
for patient in PatientCache.objects.all():
    print(f"{patient.patient_id} → {patient.openemr_patient_id}")

# Outbox 상태 확인
outbox = SyncOutbox.objects.filter(entity_type='patient')
print(f"Outbox records: {outbox.count()}")
for record in outbox:
    print(f"  {record.entity_id}: {record.status}")
```

**예상 출력**:
```
Total patients: 5
P-2025-001 → OPENEMR_P-2025-001
P-2025-002 → OPENEMR_P-2025-002
P-2025-003 → OPENEMR_P-2025-003
P-2025-004 → OPENEMR_P-2025-004
P-2025-005 → OPENEMR_P-2025-005

Outbox records: 5
  P-2025-001: done
  P-2025-002: done
  P-2025-003: done
  P-2025-004: done
  P-2025-005: done
```

---

### 2. OpenEMR 확인

```powershell
# OpenEMR 컨테이너 내부 MySQL 접속
docker-compose -f docker-compose.dev.yml exec openemr-mysql mysql -u openemr -popenemr openemr

# SQL 쿼리 실행
SELECT id, pubpid, fname, lname, DOB, sex
FROM patient_data
WHERE pubpid LIKE 'OPENEMR_P-2025-%'
ORDER BY pubpid;
```

**예상 출력**:
```
+-----+---------------------+---------+----------+------------+------+
| id  | pubpid              | fname   | lname    | DOB        | sex  |
+-----+---------------------+---------+----------+------------+------+
| 456 | OPENEMR_P-2025-001  | MinJun  | Kim      | 1985-05-15 | Male |
| 457 | OPENEMR_P-2025-002  | SeoYun  | Lee      | 1990-08-22 | Female |
| 458 | OPENEMR_P-2025-003  | JiHoon  | Park     | 1978-03-10 | Male |
| 459 | OPENEMR_P-2025-004  | YeJin   | Choi     | 1995-11-30 | Female |
| 460 | OPENEMR_P-2025-005  | DoHyun  | Jung     | 1982-07-18 | Male |
+-----+---------------------+---------+----------+------------+------+
```

---

### 3. HAPI FHIR 확인

```powershell
# FHIR 서버에서 Patient 조회
curl http://localhost:8080/fhir/Patient?identifier=P-2025-001
```

**예상 응답** (JSON):
```json
{
  "resourceType": "Bundle",
  "type": "searchset",
  "total": 1,
  "entry": [
    {
      "resource": {
        "resourceType": "Patient",
        "id": "fhir-123",
        "identifier": [
          {
            "system": "http://neuronova.com/patient-id",
            "value": "P-2025-001"
          }
        ],
        "name": [
          {
            "family": "Kim",
            "given": ["MinJun"]
          }
        ],
        "gender": "male",
        "birthDate": "1985-05-15"
      }
    }
  ]
}
```

---

### 4. Orthanc PACS 확인

```powershell
# Orthanc 헬스체크
curl http://localhost:8042/system

# 저장된 Studies 수 확인
curl http://localhost:8042/statistics
```

**예상 응답**:
```json
{
  "TotalDiskSize": "2048MB",
  "TotalDiskSizeMB": 2048,
  "CountPatients": 2,
  "CountStudies": 2,
  "CountSeries": 20,
  "CountInstances": 1860
}
```

---

## 🔧 트러블슈팅

### 문제 1: OpenEMR 동기화 실패

**증상**: Outbox 레코드가 `failed` 상태

**원인**:
- OpenEMR 컨테이너 미실행
- OpenEMR FHIR API 접근 불가
- 네트워크 타임아웃

**해결**:
```powershell
# OpenEMR 컨테이너 상태 확인
docker-compose -f docker-compose.dev.yml ps openemr

# OpenEMR 로그 확인
docker-compose -f docker-compose.dev.yml logs openemr

# OpenEMR FHIR API 접근 테스트
curl http://localhost:80/apis/default/fhir/Patient

# 수동 재시도
docker-compose -f docker-compose.dev.yml exec django python manage.py retry_failed_outbox
```

---

### 문제 2: FHIR 자동 동기화 실패

**증상**: `fhir_id` 필드가 `None`

**원인**:
- FHIR signals 비활성화
- HAPI FHIR 서버 다운

**해결**:
```powershell
# FHIR signals 활성화 확인
docker-compose -f docker-compose.dev.yml exec django python manage.py shell

# Python shell
from fhir.apps import FhirConfig
print(FhirConfig.ready)  # Should print True

# HAPI FHIR 서버 헬스체크
curl http://localhost:8080/fhir/metadata

# HAPI FHIR 컨테이너 재시작
docker-compose -f docker-compose.dev.yml restart hapi-fhir
```

---

### 문제 3: Celery Worker 미실행

**증상**: Outbox 레코드가 계속 `pending` 상태

**원인**: Celery Worker 컨테이너 미실행

**해결**:
```powershell
# Celery Worker 상태 확인
docker-compose -f docker-compose.dev.yml ps celery-worker

# Celery Worker 로그 확인
docker-compose -f docker-compose.dev.yml logs celery-worker

# Celery Worker 재시작
docker-compose -f docker-compose.dev.yml restart celery-worker

# Celery Beat 확인 (재시도 스케줄러)
docker-compose -f docker-compose.dev.yml ps celery-beat
docker-compose -f docker-compose.dev.yml logs celery-beat
```

---

### 문제 4: 환자 ID 중복

**증상**: `IntegrityError: Duplicate entry for key 'PRIMARY'`

**원인**: 기존 데이터 존재

**해결**:
```powershell
# --reset 플래그로 기존 데이터 삭제 후 재생성
docker-compose -f docker-compose.dev.yml exec django python manage.py seed_initial_data --reset

# 또는 수동 삭제
docker-compose -f docker-compose.dev.yml exec django python manage.py shell

# Python shell
from emr.models import PatientCache
PatientCache.objects.all().delete()
```

---

### 문제 5: DICOM 업로드 실패

**증상**: `Connection refused to Orthanc`

**해결**:
```powershell
# Orthanc 컨테이너 상태 확인
docker-compose -f docker-compose.dev.yml ps orthanc

# Orthanc 헬스체크
curl http://localhost:8042/system

# Orthanc 로그 확인
docker-compose -f docker-compose.dev.yml logs orthanc

# Orthanc 재시작
docker-compose -f docker-compose.dev.yml restart orthanc
```

---

## 📊 완전 초기화 시나리오

Docker를 완전히 삭제하고 재설치한 경우:

```powershell
# 1. 루트 디렉토리로 이동
cd d:\1222\NeuroNova_v1

# 2. 기존 Docker 완전 정리 (주의!)
docker-compose -f docker-compose.dev.yml down -v

# 3. 전체 스택 시작
docker-compose -f docker-compose.dev.yml up -d

# 4. 컨테이너 상태 확인 (모두 healthy 대기)
docker-compose -f docker-compose.dev.yml ps

# 5. 초기화 대기 (Django 마이그레이션 자동 실행)
# 약 30초 대기

# 6. 테스트 사용자 생성
docker-compose -f docker-compose.dev.yml exec django python manage.py create_test_users

# 7. 샘플 데이터 생성
docker-compose -f docker-compose.dev.yml exec django python manage.py seed_initial_data

# 8. 동기화 상태 확인 (1-2분 대기)
docker-compose -f docker-compose.dev.yml exec django python manage.py check_sync_status

# 9. DICOM 업로드 (선택사항)
docker-compose -f docker-compose.dev.yml exec django python manage.py upload_sample_dicoms --dry-run

# 완료!
```

**예상 시간**: 5-7분 (컨테이너 시작 + 동기화 포함)

---

## 📚 관련 문서

- [43_데이터_흐름_아키텍처.md](43_데이터_흐름_아키텍처.md) - 데이터 저장소 구조 상세
- [44_데이터_흐름_가이드.md](44_데이터_흐름_가이드.md) - 데이터 흐름 시나리오 상세
- [19_Write_Through_패턴_가이드.md](19_Write_Through_패턴_가이드.md) - Write-Through 패턴 설명
- [39_Docker_아키텍처.md](39_Docker_아키텍처.md) - Docker 아키텍처 상세
- [40_Docker_개발_가이드.md](40_Docker_개발_가이드.md) - Docker 개발 실습

---

## 📝 폐기된 문서

다음 문서는 본 가이드로 통합되어 **폐기**되었습니다:
- ~~`16_데이터_시딩_가이드.md`~~ (레거시 환경용, 2025-12-23)
- ~~`38_데이터_초기화_가이드.md`~~ (OpenEMR 누락, 2025-12-30)

**폐기 사유**:
1. 두 문서 간 환자 ID 형식 불일치 (`P-2025-001` vs `P20250001`)
2. 38번 문서에서 OpenEMR 매핑 필드 누락
3. 데이터 아키텍처 (43, 44번 문서)를 준수하지 않음
4. Outbox 패턴 미반영

---

**문서 버전**: 1.1 (최신 수정: 2026-01-02)
**수정 사항**:
- v3.0 통합 시딩 스크립트 (`seed_minimal.py`, `seed_full_system_test_data.py`) 추가
- RIS `patient_id` 및 Order `ordered_by` DB 컬럼 매핑 정보 반영
- BRCA1 유전 변이 예제 데이터 시나리오 포함
**작성자**: NeuroNova Development Team (with Antigravity AI)
