# 배포 전 보안 체크리스트

**목적**: 프로덕션 배포 전 필수 보안 점검 항목
**대상**: NeuroNova CDSS v2.0
**최종 수정일**: 2025-12-31

---

## ⚠️ CRITICAL - 배포 전 필수 확인

이 체크리스트의 모든 항목이 완료되기 전에는 **절대 프로덕션 배포 금지**.

---

## 🔐 1. 인증 및 권한 (Authentication & Authorization)

### 1.1 비밀번호 정책

- [ ] **테스트 계정 비활성화 또는 삭제**
  ```bash
  # 모든 테스트 계정 확인
  docker exec neuronova-django-dev python manage.py shell -c "
  from acct.models import User
  test_users = User.objects.filter(
      username__in=['admin', 'doctor', 'nurse', 'patient', 'radiologist', 'labtech']
  )
  for user in test_users:
      user.is_active = False
      user.save()
  print(f'Deactivated {test_users.count()} test accounts')
  "
  ```

- [ ] **비밀번호 복잡도 요구사항 설정**
  ```python
  # settings.py
  AUTH_PASSWORD_VALIDATORS = [
      {
          'NAME': 'django.contrib.auth.password_validation.UserAttributeSimilarityValidator',
      },
      {
          'NAME': 'django.contrib.auth.password_validation.MinimumLengthValidator',
          'OPTIONS': {
              'min_length': 12,  # 최소 12자
          }
      },
      {
          'NAME': 'django.contrib.auth.password_validation.CommonPasswordValidator',
      },
      {
          'NAME': 'django.contrib.auth.password_validation.NumericPasswordValidator',
      },
  ]
  ```

- [ ] **초기 Admin 비밀번호 변경**
  ```bash
  docker exec neuronova-django-dev python manage.py changepassword admin
  ```

### 1.2 JWT 토큰 설정

- [ ] **JWT 만료 시간 단축**
  ```python
  # settings.py
  SIMPLE_JWT = {
      'ACCESS_TOKEN_LIFETIME': timedelta(hours=1),   # 24h → 1h
      'REFRESH_TOKEN_LIFETIME': timedelta(days=7),   # 30d → 7d
      'ROTATE_REFRESH_TOKENS': True,                 # 리프레시 토큰 자동 갱신
      'BLACKLIST_AFTER_ROTATION': True,              # 이전 토큰 블랙리스트
  }
  ```

- [ ] **JWT SECRET_KEY 환경변수 분리**
  ```python
  # settings.py
  JWT_SECRET_KEY = os.getenv('JWT_SECRET_KEY')
  if not JWT_SECRET_KEY:
      raise EnvironmentError('JWT_SECRET_KEY must be set in production')
  ```

### 1.3 보안 모드 활성화

- [ ] **ENABLE_SECURITY = True 설정**
  ```python
  # settings.py
  ENABLE_SECURITY = os.getenv('ENABLE_SECURITY', 'True') == 'True'
  ```

- [ ] **모든 API 엔드포인트에 권한 검증 확인**
  ```bash
  # 권한이 없는 API 검색
  grep -r "permission_classes = \[\]" NeuroNova_02_back_end/02_django_server/*/views.py
  # 결과가 없어야 함
  ```

---

## 🔒 2. 환경 변수 및 시크릿 (Secrets Management)

### 2.1 Django SECRET_KEY

- [ ] **새로운 SECRET_KEY 생성**
  ```bash
  python -c 'from django.core.management.utils import get_random_secret_key; print(get_random_secret_key())'
  ```

- [ ] **SECRET_KEY 환경변수로 설정**
  ```bash
  # .env (프로덕션 서버)
  DJANGO_SECRET_KEY="your-new-secret-key-here-min-50-chars"
  ```

- [ ] **하드코딩된 SECRET_KEY 제거**
  ```bash
  # settings.py에서 SECRET_KEY = 'xxx' 형태가 없는지 확인
  grep "SECRET_KEY = " NeuroNova_02_back_end/02_django_server/cdss_backend/settings.py
  ```

### 2.2 데이터베이스 비밀번호

- [ ] **DB 비밀번호 변경**
  ```sql
  -- MySQL
  ALTER USER 'cdss_user'@'%' IDENTIFIED BY 'new-strong-password';
  FLUSH PRIVILEGES;
  ```

- [ ] **DB 비밀번호 환경변수로 설정**
  ```bash
  # .env
  DB_PASSWORD="your-new-db-password-here"
  ```

### 2.3 외부 시스템 네트워크 격리 및 통합
- [ ] **OpenEMR/Orthanc 외부 포트 차단**: `docker-compose.yml`에서 `ports` 섹션이 제거되었는지 확인 (내부 네트워크 전용)
- [ ] **전용 Client 클래스 사용**: 모든 외부 시스템(OpenEMR, Orthanc, HAPI FHIR) 호출이 `clients/` 내 전용 클래스를 통해서만 이루어지는지 확인
- [ ] **감사 로그 기록**: 외부 시스템 접근 시 `AuditClient.log_event()`를 호출하여 접근 기록을 남기는지 확인
- [ ] **Nginx 직접 노출 금지**: `nginx.conf`에서 외부 시스템에 대한 직접적인 `location` 블록이 없는지 확인

---

## 🛡️ 3. Django 보안 설정

### 3.1 DEBUG 모드

- [ ] **DEBUG = False 설정**
  ```python
  # settings.py
  DEBUG = os.getenv('DEBUG', 'False') == 'True'

  if not DEBUG and not os.getenv('DJANGO_SECRET_KEY'):
      raise EnvironmentError('SECRET_KEY must be set when DEBUG=False')
  ```

### 3.2 ALLOWED_HOSTS

- [ ] **ALLOWED_HOSTS 명시**
  ```python
  # settings.py
  ALLOWED_HOSTS = os.getenv('ALLOWED_HOSTS', '').split(',')

  # .env (프로덕션)
  ALLOWED_HOSTS="neuronova.example.com,api.neuronova.com"
  ```

### 3.3 CORS 설정

- [ ] **CORS_ALLOWED_ORIGINS 화이트리스트**
  ```python
  # settings.py
  CORS_ALLOWED_ORIGINS = [
      "https://neuronova.example.com",
      "https://app.neuronova.com",
  ]

  # CORS_ALLOW_ALL_ORIGINS = False  # 절대 True 금지
  ```

### 3.4 보안 헤더

- [ ] **SECURE_* 설정 활성화**
  ```python
  # settings.py
  SECURE_SSL_REDIRECT = True
  SECURE_HSTS_SECONDS = 31536000  # 1년
  SECURE_HSTS_INCLUDE_SUBDOMAINS = True
  SECURE_HSTS_PRELOAD = True
  SECURE_CONTENT_TYPE_NOSNIFF = True
  SECURE_BROWSER_XSS_FILTER = True
  X_FRAME_OPTIONS = 'DENY'
  SESSION_COOKIE_SECURE = True
  CSRF_COOKIE_SECURE = True
  ```

---

## 📊 4. 데이터베이스 보안

### 4.1 백업

- [ ] **자동 백업 설정**
  ```bash
  # Cron 작업 추가 (매일 오전 3시)
  0 3 * * * /usr/local/bin/backup_cdss_db.sh
  ```

- [ ] **백업 테스트 (복원 가능한지 확인)**
  ```bash
  # 백업 생성
  docker exec neuronova-cdss-mysql mysqldump -u root -p cdss_db > backup_$(date +%Y%m%d).sql

  # 테스트 DB에 복원 테스트
  docker exec -i neuronova-cdss-mysql mysql -u root -p test_db < backup_20250101.sql
  ```

### 4.2 접근 제어

- [ ] **데이터베이스 외부 접근 차단**
  ```yaml
  # docker-compose.yml
  services:
    cdss-mysql:
      ports:
        # - "3306:3306"  # 주석 처리하여 외부 접근 차단
      networks:
        - internal  # 내부 네트워크만 허용
  ```

- [ ] **불필요한 DB 사용자 삭제**
  ```sql
  SELECT User, Host FROM mysql.user;
  -- root@%, test@% 등 불필요한 계정 삭제
  DROP USER 'test'@'%';
  ```

---

## 🌐 5. 네트워크 보안

### 5.1 방화벽 설정

- [ ] **필요한 포트만 개방**
  ```bash
  # GCP VM 방화벽 규칙
  # 80/tcp (HTTP - Redirect to HTTPS)
  # 443/tcp (HTTPS)
  # 22/tcp (SSH - IP 제한)

  # 차단할 포트:
  # 8000 (Django - Nginx만 접근)
  # 8042 (Orthanc - 내부 전용)
  # 3306 (MySQL - 내부 전용)
  # 6379 (Redis - 내부 전용)
  ```

### 5.2 HTTPS 설정

- [ ] **SSL/TLS 인증서 설치 (Let's Encrypt)**
  ```bash
  # Certbot 설치
  sudo apt install certbot python3-certbot-nginx

  # 인증서 발급
  sudo certbot --nginx -d neuronova.example.com
  ```

- [ ] **Nginx HTTPS 리디렉션 설정**
  ```nginx
  # nginx.conf
  server {
      listen 80;
      server_name neuronova.example.com;
      return 301 https://$server_name$request_uri;
  }
  ```

---

## 📝 6. 로깅 및 모니터링

### 6.1 민감 정보 로깅 방지

- [ ] **비밀번호, 토큰 로깅 제외**
  ```python
  # logging 설정
  import logging

  class SensitiveDataFilter(logging.Filter):
      def filter(self, record):
          # 비밀번호, 토큰 마스킹
          if hasattr(record, 'msg'):
              record.msg = record.msg.replace(password, '***')
          return True
  ```

### 6.2 접근 로그 보관

- [ ] **Nginx 접근 로그 보관 기간 설정 (최소 3개월)**
  ```nginx
  # nginx.conf
  access_log /var/log/nginx/access.log combined;
  error_log /var/log/nginx/error.log;

  # logrotate 설정
  /var/log/nginx/*.log {
      daily
      rotate 90
      compress
      delaycompress
  }
  ```

---

## 🧪 7. 코드 보안

### 7.1 의존성 취약점 검사

- [ ] **pip-audit 실행**
  ```bash
  pip install pip-audit
  pip-audit
  ```

- [ ] **npm audit 실행**
  ```bash
  cd NeuroNova_03_front_end_react/00_test_client
  npm audit
  npm audit fix
  ```

### 7.2 SQL Injection 방지

- [ ] **Raw SQL 사용 여부 확인**
  ```bash
  # Raw SQL 검색
  grep -r "\.raw(" NeuroNova_02_back_end/02_django_server/
  grep -r "\.execute(" NeuroNova_02_back_end/02_django_server/

  # 발견 시 Django ORM으로 변경
  ```

### 7.3 XSS 방지

- [ ] **템플릿에서 |safe 필터 사용 확인**
  ```bash
  grep -r "|safe" NeuroNova_02_back_end/02_django_server/*/templates/

  # 발견 시 필요성 재검토
  ```

---

## 📦 8. 파일 및 정적 자산

### 8.1 업로드 파일 검증

- [ ] **파일 업로드 크기 제한**
  ```python
  # settings.py
  DATA_UPLOAD_MAX_MEMORY_SIZE = 10 * 1024 * 1024  # 10MB
  FILE_UPLOAD_MAX_MEMORY_SIZE = 10 * 1024 * 1024
  ```

- [ ] **DICOM 파일 검증**
  ```python
  # ris/views.py
  import pydicom

  def upload_dicom(request):
      try:
          ds = pydicom.dcmread(file)
      except:
          return Response({"error": "Invalid DICOM file"}, 400)
  ```

---

## ✅ 배포 전 최종 체크리스트

### Phase 1: 설정 확인
- [ ] DEBUG = False
- [ ] SECRET_KEY 환경변수 설정
- [ ] ALLOWED_HOSTS 설정
- [ ] DB 비밀번호 변경
- [ ] JWT 토큰 만료 시간 단축

### Phase 2: 보안 점검
- [ ] 테스트 계정 비활성화
- [ ] HTTPS 인증서 설치
- [ ] 방화벽 규칙 설정
- [ ] 의존성 취약점 검사
- [ ] 백업 테스트

### Phase 3: 모니터링 설정
- [ ] Prometheus 알림 설정
- [ ] Grafana 대시보드 확인
- [ ] 로그 보관 정책 설정

### Phase 4: 최종 검증
- [ ] 프로덕션 환경 smoke test
- [ ] 로그인 기능 테스트
- [ ] API 응답 시간 확인
- [ ] 에러 발생 시 알림 수신 테스트

---

## 🚨 배포 후 모니터링

### 첫 24시간
- [ ] 매 1시간마다 로그 확인
- [ ] 접근 패턴 이상 징후 모니터링
- [ ] 데이터베이스 성능 확인

### 첫 1주일
- [ ] 매일 보안 로그 검토
- [ ] 인증 실패 시도 확인
- [ ] 성능 지표 모니터링

---

**작성**: Claude AI
**최종 업데이트**: 2025-12-31
**버전**: 1.0

**CRITICAL**: 이 체크리스트의 모든 항목이 완료되지 않으면 프로덕션 배포 금지!
